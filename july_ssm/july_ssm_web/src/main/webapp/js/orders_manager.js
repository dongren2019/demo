...
function load(pageNum,pageSize) {
    $.ajax({
        url:'/orders/find_all?pageSize='+pageSize+"&pageNum="+pageNum,
        type:'get',
        dataType:'json',
        success:function(result){
            ...
            for(var p in data){//遍历json数组时，这么写p为索引，0,1
                ls+='<tr>\n' +
                    ...
                    '                                <td>'+data[p].id+'</td>\n' +
                    '                                <td>'+data[p].orderNum+'</td>\n' +
                    '                                <td>'+data[p].product.productName+'</td>\n' +
                    '                                <td>'+data[p].product.productPrice+'</td>\n' +
                    '                                <td>'+data[p].orderTimeStr+'</td>\n' +
                    '                                <td>'+data[p].orderStatusStr+'</td>\n' +
                    '\n' +
                    '                                <td class="text-center">\n' +
                    '                                    <button type="button" class="btn bg-olive btn-xs" onclick=\'location.href="all-order-manage-edit.html"\'>订单</button>\n' +
                    '                                    <button type="button" class="btn bg-olive btn-xs" onclick="showOrdersDetailPage('+data[p].id+')">详情</button>\n' +
...
showOrdersDetailPage = function (id) {
    location.href="/orders/showOrdersDetailPage?id="+id;
}
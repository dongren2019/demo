...
function load(pageNum,pageSize) {
    $.ajax({
        url:'/role/find_all?pageSize='+pageSize+"&pageNum="+pageNum,
        type:'get',
        dataType:'json',
        success:function(result){
            ...
            for(var p in data){//遍历json数组时，这么写p为索引，0,1
                ls+='<tr>\n' +
                    ...
                    '                                    <button type="button" class="btn bg-olive btn-xs" onclick="showAddPermissionsToRolePage('+data[p].id+')"">添加权限</button>\n' +
                    '                                </td>\n' +
                    '                            </tr>'
            }
            tbody.html(ls);
...
showRoleDetailPage = function (id) {
    location.href="/role/show_userDeatil_page?id="+id;
}

showAddPermissionsToRolePage = function (id) {

...
function load(pageNum,pageSize) {
    $.ajax({
        url:'/permission/find_all?pageSize='+pageSize+"&pageNum="+pageNum,
        type:'get',
        dataType:'json',
        success:function(result){
            ...
            for(var p in data){//遍历json数组时，这么写p为索引，0,1
                ls+='<tr>\n' +
                    ...
                    '                                    <button type="button" class="btn bg-olive btn-xs" onclick="showPermissionDetailPage('+data[p].id+')">详情</button>\n' +
                    '                                    <button type="button" class="btn bg-olive btn-xs" onclick="">添加角色</button>\n' +
                    '                                </td>\n' +
                    '                            </tr>'
            }
            tbody.html(ls);
